...
Phase 6 已从 Session 装配骨架推进到 native 资源句柄闭环：生命周期状态、登录凭据、transport 配置生成、storage target 生成、领域服务聚合、监听器注册表、任务监督器、资源适配器边界、登录态资源清理、Session 运行时资源句柄、native SQLite 打开与迁移、transport/sync 任务句柄和单元测试均已落地。本轮继续把 native transport 从“只有 connect_url 的占位句柄”推进到“登录时真实创建 `NativeWsClient` 连接资源、后台 worker 轮询收包并在断线后自动重连、退出时主动 close”，同时补上基于 `/auth/parse_token` 的本地 HTTP 登录校验，在 fixture 下验证 token、userID 和 platformID 对齐。当前实现仍不冒充完整长连链路：wasm IndexedDB 运行时装配、transport 事件到 session/领域层派发、平台回调线程和真实 API endpoint 联调仍属于后续 Gate。
...
<!-- code-ref: phase6-native-transport-open -> file:///Volumes/ssd/Users/hj/Documents/code/github/openim/openim-sdk-core-rust/crates/openim-session-native/src/lib.rs#L241 -->
...
<!-- code-ref: phase6-native-transport-close -> file:///Volumes/ssd/Users/hj/Documents/code/github/openim/openim-sdk-core-rust/crates/openim-session-native/src/lib.rs#L224 -->
...
<!-- code-ref: phase6-native-transport-worker -> file:///Volumes/ssd/Users/hj/Documents/code/github/openim/openim-sdk-core-rust/crates/openim-session-native/src/lib.rs#L268 -->
<!-- code-ref: phase6-native-reconnect-policy -> file:///Volumes/ssd/Users/hj/Documents/code/github/openim/openim-sdk-core-rust/crates/openim-session-native/src/lib.rs#L303 -->

- <code style="background:#FFF4E5;color:#C2410C;padding:0 0.2em;border-radius:4px;" data-code-ref="phase6-native-resource-test">native_adapter_opens_sqlite_storage_and_closes_resources_on_logout</code>、<code style="background:#FFF4E5;color:#C2410C;padding:0 0.2em;border-radius:4px;" data-code-ref="phase6-http-login-test">native_adapter_rejects_parse_token_user_mismatch</code>、<code style="background:#FFF4E5;color:#C2410C;padding:0 0.2em;border-radius:4px;" data-code-ref="phase6-native-reconnect-test">native_adapter_reconnects_transport_after_disconnect</code>、<code style="background:#FFF4E5;color:#C2410C;padding:0 0.2em;border-radius:4px;" data-code-ref="phase6-native-transport-fixture">spawn_transport_server</code>、<code style="background:#FFF4E5;color:#C2410C;padding:0 0.2em;border-radius:4px;" data-code-ref="phase6-native-reconnect-fixture">spawn_reconnecting_transport_server</code> 和 <code style="background:#FFF4E5;color:#C2410C;padding:0 0.2em;border-radius:4px;" data-code-ref="phase6-http-login-fixture">spawn_parse_token_server</code> 一起验证登录时 SQLite 文件真实创建、本地 WebSocket 连接成功建立、HTTP 登录校验会拦截错误 userID、本地断线后会发起第二次连接，以及 Logout 后 runtime resources 被清空。
<!-- code-ref: phase6-native-resource-test -> file:///Volumes/ssd/Users/hj/Documents/code/github/openim/openim-sdk-core-rust/crates/openim-session-native/src/lib.rs#L430 -->
<!-- code-ref: phase6-http-login-test -> file:///Volumes/ssd/Users/hj/Documents/code/github/openim/openim-sdk-core-rust/crates/openim-session-native/src/lib.rs#L488 -->
<!-- code-ref: phase6-native-reconnect-test -> file:///Volumes/ssd/Users/hj/Documents/code/github/openim/openim-sdk-core-rust/crates/openim-session-native/src/lib.rs#L510 -->
<!-- code-ref: phase6-native-transport-fixture -> file:///Volumes/ssd/Users/hj/Documents/code/github/openim/openim-sdk-core-rust/crates/openim-session-native/src/lib.rs#L532 -->
<!-- code-ref: phase6-native-reconnect-fixture -> file:///Volumes/ssd/Users/hj/Documents/code/github/openim/openim-sdk-core-rust/crates/openim-session-native/src/lib.rs#L565 -->
<!-- code-ref: phase6-http-login-fixture -> file:///Volumes/ssd/Users/hj/Documents/code/github/openim/openim-sdk-core-rust/crates/openim-session-native/src/lib.rs#L606 -->
...
当前已完成：Init、Login、Logout、UnInit 状态转换；登录前置校验；transport config 生成；SQLite 与 IndexedDB storage target 生成；登录态 user id 保存；登录态领域服务重置；资源适配器注入；Init/Login/Logout/UnInit 资源边界回调；SessionRuntimeResources 句柄持有；ResourceOpened/ResourceClosed 事件；Logout 和 UnInit 关闭资源句柄；native SQLite 父目录创建、数据库打开和 migration；`/auth/parse_token` HTTP 登录校验；native `NativeWsClient` 连接资源创建与 close；native transport 后台轮询与本地断线重连；sync 任务资源句柄；登出和反初始化任务清理；监听器注册和注销；注销后回调隔离；任务启停事件；重复 Logout 幂等；Session crate 单元测试、native session adapter 单元测试、transport-native 单元测试和 workspace check。

当前未完成：transport 事件到 session/领域层派发、wasm IndexedDB 运行时打开和关闭、真实同步任务调度、平台线程切换策略、监听器类型细分、真实 API endpoint 联调和前后台生命周期回归。因此 Phase 6 已具备 native 本地资源句柄闭环、本地 transport 连接资源闭环、本地 HTTP 登录校验闭环和本地断线重连 worker，但完整跨平台 Session Gate 仍需后续真实资源和平台运行器验证。
