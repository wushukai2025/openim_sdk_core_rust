...
Phase 6 已从 Session 装配骨架推进到 native 资源句柄闭环：生命周期状态、登录凭据、transport 配置生成、storage target 生成、领域服务聚合、监听器注册表、任务监督器、资源适配器边界、登录态资源清理、Session 运行时资源句柄、native SQLite 打开与迁移、transport/sync 任务句柄和单元测试均已落地。本轮继续把 native transport 从“只有 connect_url 的占位句柄”推进到“登录时真实创建 NativeWsClient 连接资源、后台 worker 轮询收包并在断线后自动重连、退出时主动 close”，同时补上基于 /auth/parse_token 的本地 HTTP 登录校验，并进一步接通本地 PushMsg 文本消息从 transport worker 到 session/领域事件的 drain/pump 链路。当前实现仍不冒充完整长连链路：wasm IndexedDB 运行时装配、PushMsg 以外的 transport 事件接入、更多消息内容类型转换、平台回调线程和真实 API endpoint 联调仍属于后续 Gate。
...
- <code style="background:#FFF4E5;color:#C2410C;padding:0 0.2em;border-radius:4px;" data-code-ref="phase6-resource-handle">SessionResourceHandle</code> 已补 drain_transport_events 默认入口，<code style="background:#FFF4E5;color:#C2410C;padding:0 0.2em;border-radius:4px;" data-code-ref="phase6-runtime-drain-events">SessionRuntimeResources::drain_transport_events</code> 会聚合所有资源句柄产出的 transport 事件，作为后续 session 主线程显式 pump 的边界。
<!-- code-ref: phase6-resource-handle -> file:///Volumes/ssd/Users/hj/Documents/code/github/openim/openim-sdk-core-rust/crates/openim-session/src/lib.rs#L230 -->
<!-- code-ref: phase6-runtime-drain-events -> file:///Volumes/ssd/Users/hj/Documents/code/github/openim/openim-sdk-core-rust/crates/openim-session/src/lib.rs#L378 -->
...
- <code style="background:#FFF4E5;color:#C2410C;padding:0 0.2em;border-radius:4px;" data-code-ref="phase6-pump-transport-events">pump_transport_events</code> 会从 runtime resources drain 当前积累的 transport 事件，并把 PushMsg 解成领域消息后复用现有会话投影和 session listener 派发；当前本地 Gate 只覆盖文本消息和 typing 过滤，不冒充完整消息类型矩阵。
<!-- code-ref: phase6-pump-transport-events -> file:///Volumes/ssd/Users/hj/Documents/code/github/openim/openim-sdk-core-rust/crates/openim-session/src/lib.rs#L797 -->

- <code style="background:#FFF4E5;color:#C2410C;padding:0 0.2em;border-radius:4px;" data-code-ref="phase6-decode-transport-message">decode_transport_message</code> 基于 <code style="background:#FFF4E5;color:#C2410C;padding:0 0.2em;border-radius:4px;" data-code-ref="phase6-incoming-transport-message">IncomingTransportMessage</code> 把 transport push 负载转换成领域 ChatMessage；当前先支持 Text，Typing 会被忽略，其他 content type 保持显式报错，避免静默丢消息。
<!-- code-ref: phase6-decode-transport-message -> file:///Volumes/ssd/Users/hj/Documents/code/github/openim/openim-sdk-core-rust/crates/openim-session/src/lib.rs#L973 -->
<!-- code-ref: phase6-incoming-transport-message -> file:///Volumes/ssd/Users/hj/Documents/code/github/openim/openim-sdk-core-rust/crates/openim-transport-core/src/lib.rs#L90 -->
...
- <code style="background:#FFF4E5;color:#C2410C;padding:0 0.2em;border-radius:4px;" data-code-ref="phase6-native-transport-open">start_native_transport_task</code> 会创建 worker 线程、建立 current-thread Tokio runtime，并在登录阶段直接建立 NativeWsClient 连接；连接失败会按 network error 返回，避免 session 把无效 transport 当作已登录资源。
<!-- code-ref: phase6-native-transport-open -> file:///Volumes/ssd/Users/hj/Documents/code/github/openim/openim-sdk-core-rust/crates/openim-session-native/src/lib.rs#L250 -->
...
<!-- code-ref: phase6-native-transport-close -> file:///Volumes/ssd/Users/hj/Documents/code/github/openim/openim-sdk-core-rust/crates/openim-session-native/src/lib.rs#L225 -->

- <code style="background:#FFF4E5;color:#C2410C;padding:0 0.2em;border-radius:4px;" data-code-ref="phase6-native-transport-drain">NativeTransportTaskResource::drain_transport_events</code> 会把 worker 线程通过 channel 传回的 transport 事件批量交给 session 主线程处理，避免在 native worker 内直接持有领域状态。
<!-- code-ref: phase6-native-transport-drain -> file:///Volumes/ssd/Users/hj/Documents/code/github/openim/openim-sdk-core-rust/crates/openim-session-native/src/lib.rs#L241 -->

- <code style="background:#FFF4E5;color:#C2410C;padding:0 0.2em;border-radius:4px;" data-code-ref="phase6-native-transport-worker">run_native_transport_worker</code> 会在独立线程里轮询 recv_event，在收到 Disconnected 后按默认 <code style="background:#FFF4E5;color:#C2410C;padding:0 0.2em;border-radius:4px;" data-code-ref="phase6-native-reconnect-policy">ReconnectPolicy</code> 自动重连，并在 shutdown 信号到达后中断轮询并关闭连接。
<!-- code-ref: phase6-native-transport-worker -> file:///Volumes/ssd/Users/hj/Documents/code/github/openim/openim-sdk-core-rust/crates/openim-session-native/src/lib.rs#L280 -->
<!-- code-ref: phase6-native-reconnect-policy -> file:///Volumes/ssd/Users/hj/Documents/code/github/openim/openim-sdk-core-rust/crates/openim-session-native/src/lib.rs#L315 -->

- <code style="background:#FFF4E5;color:#C2410C;padding:0 0.2em;border-radius:4px;" data-code-ref="phase6-decode-push-response">decode_push_messages_response</code> 会把 PushMsg protobuf 负载摊平成稳定排序的 transport 消息列表，作为 session 层 pump transport events 的解码入口。
<!-- code-ref: phase6-decode-push-response -> file:///Volumes/ssd/Users/hj/Documents/code/github/openim/openim-sdk-core-rust/crates/openim-transport-core/src/lib.rs#L385 -->

- <code style="background:#FFF4E5;color:#C2410C;padding:0 0.2em;border-radius:4px;" data-code-ref="phase6-native-resource-test">native_adapter_opens_sqlite_storage_and_closes_resources_on_logout</code>、<code style="background:#FFF4E5;color:#C2410C;padding:0 0.2em;border-radius:4px;" data-code-ref="phase6-http-login-test">native_adapter_rejects_parse_token_user_mismatch</code>、<code style="background:#FFF4E5;color:#C2410C;padding:0 0.2em;border-radius:4px;" data-code-ref="phase6-native-reconnect-test">native_adapter_reconnects_transport_after_disconnect</code>、<code style="background:#FFF4E5;color:#C2410C;padding:0 0.2em;border-radius:4px;" data-code-ref="phase6-native-push-test">native_adapter_pumps_transport_push_into_session_events</code>、<code style="background:#FFF4E5;color:#C2410C;padding:0 0.2em;border-radius:4px;" data-code-ref="phase6-native-transport-fixture">spawn_transport_server</code>、<code style="background:#FFF4E5;color:#C2410C;padding:0 0.2em;border-radius:4px;" data-code-ref="phase6-native-reconnect-fixture">spawn_reconnecting_transport_server</code>、<code style="background:#FFF4E5;color:#C2410C;padding:0 0.2em;border-radius:4px;" data-code-ref="phase6-native-push-fixture">spawn_push_transport_server</code> 和 <code style="background:#FFF4E5;color:#C2410C;padding:0 0.2em;border-radius:4px;" data-code-ref="phase6-http-login-fixture">spawn_parse_token_server</code> 一起验证登录时 SQLite 文件真实创建、本地 WebSocket 连接成功建立、HTTP 登录校验会拦截错误 userID、本地断线后会发起第二次连接、PushMsg 文本消息能被 pump 到 session 事件，以及 Logout 后 runtime resources 被清空。
<!-- code-ref: phase6-native-resource-test -> file:///Volumes/ssd/Users/hj/Documents/code/github/openim/openim-sdk-core-rust/crates/openim-session-native/src/lib.rs#L448 -->
<!-- code-ref: phase6-http-login-test -> file:///Volumes/ssd/Users/hj/Documents/code/github/openim/openim-sdk-core-rust/crates/openim-session-native/src/lib.rs#L506 -->
<!-- code-ref: phase6-native-reconnect-test -> file:///Volumes/ssd/Users/hj/Documents/code/github/openim/openim-sdk-core-rust/crates/openim-session-native/src/lib.rs#L528 -->
<!-- code-ref: phase6-native-push-test -> file:///Volumes/ssd/Users/hj/Documents/code/github/openim/openim-sdk-core-rust/crates/openim-session-native/src/lib.rs#L554 -->
<!-- code-ref: phase6-native-transport-fixture -> file:///Volumes/ssd/Users/hj/Documents/code/github/openim/openim-sdk-core-rust/crates/openim-session-native/src/lib.rs#L591 -->
<!-- code-ref: phase6-native-reconnect-fixture -> file:///Volumes/ssd/Users/hj/Documents/code/github/openim/openim-sdk-core-rust/crates/openim-session-native/src/lib.rs#L624 -->
<!-- code-ref: phase6-native-push-fixture -> file:///Volumes/ssd/Users/hj/Documents/code/github/openim/openim-sdk-core-rust/crates/openim-session-native/src/lib.rs#L680 -->
<!-- code-ref: phase6-http-login-fixture -> file:///Volumes/ssd/Users/hj/Documents/code/github/openim/openim-sdk-core-rust/crates/openim-session-native/src/lib.rs#L717 -->
...
当前已完成：Init、Login、Logout、UnInit 状态转换；登录前置校验；transport config 生成；SQLite 与 IndexedDB storage target 生成；登录态 user id 保存；登录态领域服务重置；资源适配器注入；Init/Login/Logout/UnInit 资源边界回调；SessionRuntimeResources 句柄持有；ResourceOpened/ResourceClosed 事件；Logout 和 UnInit 关闭资源句柄；native SQLite 父目录创建、数据库打开和 migration；/auth/parse_token HTTP 登录校验；native NativeWsClient 连接资源创建与 close；native transport 后台轮询与本地断线重连；native worker transport event drain；PushMsg 文本消息到 session/领域事件的显式 pump 链路；sync 任务资源句柄；登出和反初始化任务清理；监听器注册和注销；注销后回调隔离；任务启停事件；重复 Logout 幂等；Session crate 单元测试、native session adapter 单元测试、transport-core 单元测试、transport-native 单元测试和 workspace check。

当前未完成：PushMsg 以外的 transport 事件到 session/领域层派发、更多消息内容类型转换、wasm IndexedDB 运行时打开和关闭、真实同步任务调度、平台线程切换策略、监听器类型细分、真实 API endpoint 联调和前后台生命周期回归。因此 Phase 6 已具备 native 本地资源句柄闭环、本地 transport 连接资源闭环、本地 HTTP 登录校验闭环、本地断线重连 worker 和文本 PushMsg 事件桥，但完整跨平台 Session Gate 仍需后续真实资源和平台运行器验证。
