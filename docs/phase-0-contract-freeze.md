...
Phase 0 已补可本地验证的契约冻结骨架：公开生命周期 API 种子清单、监听器方法清单、核心事件顺序、文件上传进度事件和 Go SDK 错误码映射已经进入 Golden fixture，并新增 Rust 兼容测试 crate 固定这些最小契约。本轮继续补上 Go SDK 源码级 surface 自动抽取、replay transcript 校验入口和绑定层回调命名/线程语义冻结，当前能从 open_im_sdk 导出函数和 callback_client.go listener interface 自动生成全量 public API/listener 清单，也能校验 Go/Rust 真实回放 transcript 是否覆盖 fixture 要求的事件顺序。当前仍不是完整契约冻结：真实 Go SDK 回放数据采集、真实服务端场景 Golden Event、Rust 同场景事件输出采集和真实 transcript 对比执行仍需继续补齐。
...
当前已完成：Phase 0 第一版契约 fixture、兼容测试 crate、核心 listener surface 覆盖检查、核心 public API surface 覆盖检查、Go SDK 源码 public API 自动抽取、Go SDK 源码 listener 自动抽取、fixture 种子对 Go 源码 surface 子集检查、replay transcript 顺序校验入口、Go/Rust replay transcript 序列对比入口、真实 transcript ignored Gate、绑定层回调命名冻结、绑定层回调线程语义冻结、核心 scenario 覆盖检查、错误码数值对齐检查、登录同步顺序最小检查。

当前未完成：真实 Go SDK 自动回放采集器、真实服务端 Golden Event 采集、Rust 端同场景事件输出采集、真实 transcript 对比执行。因此 Phase 0 现在是可运行骨架，不是完整契约冻结终态。
