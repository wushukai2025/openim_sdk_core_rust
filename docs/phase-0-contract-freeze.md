...
Phase 0 已补可本地验证的契约冻结骨架：公开生命周期 API 种子清单、监听器方法清单、核心事件顺序、文件上传进度事件和 Go SDK 错误码映射已经进入 Golden fixture，并新增 Rust 兼容测试 crate 固定这些最小契约。本轮继续补上 Go SDK 源码级 surface 自动抽取、replay transcript 校验入口、绑定层回调命名/线程语义冻结、真实回放 transcript 采集工具、Rust 本地 session lifecycle 采集入口、Rust 真实 transport 采集入口、Go SDK 真实场景回放 harness 源码入口、Go harness 依赖校验文件和真实 Gate 就绪检查入口。当前能从 open_im_sdk 导出函数和 callback_client.go listener interface 自动生成全量 public API/listener 清单，也能把真实回放器、Rust 本地 session 或 Rust 真实 transport probe 输出归一成 transcript，并用命令检查真实回放所需工具链和环境变量是否齐备。当前仍不是完整契约冻结：Go 工具链已安装且 Go SDK 回放 harness 本地编译通过，但本机仍缺少真实 OpenIM 环境变量；真实服务端场景 Golden Event、Go/Rust 完整同场景事件输出采集和真实 transcript 对比执行仍需继续补齐。
...
<!-- code-ref: phase0-replay-capture-command -> file:///Volumes/ssd/Users/hj/Documents/code/github/openim/openim-sdk-core-rust/tools/replay-capture/src/main.rs#L295 -->

- <code style="background:#FFF4E5;color:#C2410C;padding:0 0.2em;border-radius:4px;" data-code-ref="phase0-replay-check-real-gate">check_real_gate</code> 检查 Go 回放 harness、Go 工具链和真实服务端环境变量是否齐备，并在缺失时返回失败作为 Phase 0 外部 Gate 的前置证据。
<!-- code-ref: phase0-replay-check-real-gate -> file:///Volumes/ssd/Users/hj/Documents/code/github/openim/openim-sdk-core-rust/tools/replay-capture/src/main.rs#L165 -->

- <code style="background:#FFF4E5;color:#C2410C;padding:0 0.2em;border-radius:4px;" data-code-ref="phase0-go-replay-harness">go-phase0-replay</code> 是 Go SDK 真实场景回放 harness 源码入口，使用真实服务端登录、同步、可选发消息和可选上传来输出 JSONL transcript。
<!-- code-ref: phase0-go-replay-harness -> file:///Volumes/ssd/Users/hj/Documents/code/github/openim/openim-sdk-core-rust/tools/go-phase0-replay/main.go#L402 -->
...
<!-- code-ref: phase0-replay-capture-jsonl -> file:///Volumes/ssd/Users/hj/Documents/code/github/openim/openim-sdk-core-rust/tools/replay-capture/src/main.rs#L318 -->
...
<!-- code-ref: phase0-replay-capture-rust-session -> file:///Volumes/ssd/Users/hj/Documents/code/github/openim/openim-sdk-core-rust/tools/replay-capture/src/main.rs#L326 -->

- <code style="background:#FFF4E5;color:#C2410C;padding:0 0.2em;border-radius:4px;" data-code-ref="phase0-replay-capture-rust-transport">capture_rust_transport</code> 输出 Rust native WebSocket 真实服务端 probe transcript，覆盖 OnConnListener 连接事件、GetNewestSeq 请求响应和可选 PushMsg 到 OnRecvNewMessage 的事件归一化；当前不会把尚未接入的 session 同步和文件上传回调伪装成完成。
<!-- code-ref: phase0-replay-capture-rust-transport -> file:///Volumes/ssd/Users/hj/Documents/code/github/openim/openim-sdk-core-rust/tools/replay-capture/src/main.rs#L332 -->
...
<!-- code-ref: phase0-replay-capture-validate -> file:///Volumes/ssd/Users/hj/Documents/code/github/openim/openim-sdk-core-rust/tools/replay-capture/src/main.rs#L342 -->
...
<!-- code-ref: phase0-replay-capture-compare -> file:///Volumes/ssd/Users/hj/Documents/code/github/openim/openim-sdk-core-rust/tools/replay-capture/src/main.rs#L262 -->
...
<!-- code-ref: phase0-replay-rust-session-events -> file:///Volumes/ssd/Users/hj/Documents/code/github/openim/openim-sdk-core-rust/tools/replay-capture/src/main.rs#L411 -->
...
- replay 采集工具：支持捕获外部命令 stdout JSONL、读取 JSONL 文件或 stdin、输出 Rust 本地 session lifecycle transcript、输出 Rust native WebSocket 真实 transport probe transcript，并用 Phase 0 fixture 校验和对比真实回放 transcript。
- Go SDK 回放 harness：已新增源码入口，读取真实服务端地址、账号、token、可选发送目标和可选上传请求，输出可交给 replay-capture 归一化的 JSONL；当前 Go 工具链已安装，harness 依赖校验文件已生成，本地 go test ./... 已通过，真实运行仍等待有效 OpenIM 服务端环境。
- Rust transport probe：已新增真实 WebSocket 采集命令，读取真实服务端地址、账号、token，连接成功后发送 GetNewestSeq，并可选择等待 PushMsg；完整 Phase 0 required transcript 仍需要后续 session 同步、文件上传和真实消息触发闭环。
- 真实 Gate 就绪检查：replay-capture 可以检查 Go harness 是否存在、Go 工具链是否可用，提示 Rust transport probe 入口存在，以及 OPENIM_API_ADDR、OPENIM_WS_ADDR、OPENIM_USER_ID、OPENIM_TOKEN 和可选 transcript 环境变量是否齐备；当前 go_tool=ok，真实服务端环境变量仍缺失。
...
(cd tools/go-phase0-replay && go test ./...)
cargo run -p replay-capture -- capture-rust-session --output rust-local-session.json
cargo run -p replay-capture -- capture-rust-transport --output rust-transport-events.json
cargo run -p replay-capture -- check-real-gate
cargo run -p replay-capture -- capture-command --output go-events.json -- /path/to/go-phase0-replay --scenario phase0
cargo run -p replay-capture -- capture-command --output go-events.json -- sh -lc 'cd tools/go-phase0-replay && go run .'
...
当前已完成：Phase 0 第一版契约 fixture、兼容测试 crate、replay-capture 工具、核心 listener surface 覆盖检查、核心 public API surface 覆盖检查、Go SDK 源码 public API 自动抽取、Go SDK 源码 listener 自动抽取、fixture 种子对 Go 源码 surface 子集检查、replay transcript 顺序校验入口、Go/Rust replay transcript 序列对比入口、真实 transcript ignored Gate、真实回放 JSONL 采集与归一化入口、Rust 本地 session lifecycle transcript 采集入口、Rust native WebSocket 真实 transport probe 采集入口、Go SDK 真实场景回放 harness 源码入口、Go harness 依赖校验文件、本地 Go harness 编译检查、真实 Gate 就绪检查入口、真实 transcript CLI 对比入口、绑定层回调命名冻结、绑定层回调线程语义冻结、核心 scenario 覆盖检查、错误码数值对齐检查、登录同步顺序最小检查。

当前未完成：Go SDK 真实场景回放 harness 真实执行、真实服务端 Golden Event 采集、Rust 真实服务端完整同场景事件输出采集、真实 transcript 对比执行。当前 <code style="background:#FFF4E5;color:#C2410C;padding:0 0.2em;border-radius:4px;" data-code-ref="phase0-replay-check-real-gate-status">check-real-gate</code> 显示 go_tool=ok，但 OPENIM_API_ADDR、OPENIM_WS_ADDR、OPENIM_USER_ID、OPENIM_TOKEN 缺失；新增 Rust transport probe 只能在真实环境下验证连接、GetNewestSeq 和可选 PushMsg，仍不能替代完整契约冻结终态。
<!-- code-ref: phase0-replay-check-real-gate-status -> file:///Volumes/ssd/Users/hj/Documents/code/github/openim/openim-sdk-core-rust/tools/replay-capture/src/main.rs#L165 -->
