...
Phase 0 的本地可验证契约冻结骨架已告一段落：公开生命周期 API 种子清单、监听器方法清单、核心事件顺序、文件上传进度事件、Go SDK 错误码映射、Go SDK 源码级 surface 自动抽取、replay transcript 校验入口、绑定层回调命名/线程语义冻结、真实回放 transcript 采集工具、Rust 本地 session lifecycle 采集入口、Rust 真实 transport 采集入口、Go SDK 真实场景回放 harness 源码入口、Go harness 依赖校验文件和真实 Gate 就绪检查入口均已落地。当前能从 open_im_sdk 导出函数和 callback_client.go listener interface 自动生成全量 public API/listener 清单，也能把真实回放器、Rust 本地 session 或 Rust 真实 transport probe 输出归一成 transcript，并用命令检查真实回放所需工具链和环境变量是否齐备。由于暂时没有真实 OpenIM 服务端地址、账号和 token，剩余真实场景 Golden Event、Go/Rust 完整同场景事件输出采集和真实 transcript 对比执行转为外部 Gate 暂挂，后续有真实环境后再恢复执行。
...
<!-- code-ref: phase0-replay-check-real-gate -> file:///Volumes/ssd/Users/hj/Documents/code/github/openim/openim-sdk-core-rust/tools/replay-capture/src/main.rs#L167 -->
...
<!-- code-ref: phase0-replay-capture-jsonl -> file:///Volumes/ssd/Users/hj/Documents/code/github/openim/openim-sdk-core-rust/tools/replay-capture/src/main.rs#L320 -->
...
<!-- code-ref: phase0-replay-capture-rust-session -> file:///Volumes/ssd/Users/hj/Documents/code/github/openim/openim-sdk-core-rust/tools/replay-capture/src/main.rs#L328 -->
...
<!-- code-ref: phase0-replay-capture-rust-transport -> file:///Volumes/ssd/Users/hj/Documents/code/github/openim/openim-sdk-core-rust/tools/replay-capture/src/main.rs#L334 -->
...
<!-- code-ref: phase0-replay-capture-validate -> file:///Volumes/ssd/Users/hj/Documents/code/github/openim/openim-sdk-core-rust/tools/replay-capture/src/main.rs#L344 -->
...
<!-- code-ref: phase0-replay-capture-compare -> file:///Volumes/ssd/Users/hj/Documents/code/github/openim/openim-sdk-core-rust/tools/replay-capture/src/main.rs#L264 -->
...
当前未完成但已暂挂为外部 Gate：Go SDK 真实场景回放 harness 真实执行、真实服务端 Golden Event 采集、Rust 真实服务端完整同场景事件输出采集、真实 transcript 对比执行。当前 <code style="background:#FFF4E5;color:#C2410C;padding:0 0.2em;border-radius:4px;" data-code-ref="phase0-replay-check-real-gate-status">check-real-gate</code> 显示 go_tool=ok，但 OPENIM_API_ADDR、OPENIM_WS_ADDR、OPENIM_USER_ID、OPENIM_TOKEN 缺失；新增 Rust transport probe 只能在真实环境下验证连接、GetNewestSeq 和可选 PushMsg，仍不能替代完整契约冻结终态。没有真实环境时不继续扩写离线替身，后续阶段优先推进可本地验证的 Session 资源装配、上传客户端边界和绑定层骨架。
<!-- code-ref: phase0-replay-check-real-gate-status -> file:///Volumes/ssd/Users/hj/Documents/code/github/openim/openim-sdk-core-rust/tools/replay-capture/src/main.rs#L167 -->
