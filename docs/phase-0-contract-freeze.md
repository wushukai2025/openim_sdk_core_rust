...
Phase 0 已补可本地验证的契约冻结骨架：公开生命周期 API 种子清单、监听器方法清单、核心事件顺序、文件上传进度事件和 Go SDK 错误码映射已经进入 Golden fixture，并新增 Rust 兼容测试 crate 固定这些最小契约。本轮继续补上 Go SDK 源码级 surface 自动抽取、replay transcript 校验入口、绑定层回调命名/线程语义冻结、真实回放 transcript 采集工具、Rust 本地 session lifecycle 采集入口和 Go SDK 真实场景回放 harness 源码入口。当前能从 open_im_sdk 导出函数和 callback_client.go listener interface 自动生成全量 public API/listener 清单，也能把真实回放器或 Rust 本地 session 输出归一成 transcript。当前仍不是完整契约冻结：本机缺少 Go 工具链，Go SDK 回放 harness 尚未编译执行；真实服务端场景 Golden Event、Rust 真实服务端同场景事件输出采集和真实 transcript 对比执行仍需继续补齐。
