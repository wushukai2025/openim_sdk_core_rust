...
| R2-04 | Phase 6 | 本地已推进 | 已接入 SessionRuntimeResources、ResourceOpened/ResourceClosed 事件和 native Session resource adapter；native 登录会打开 SQLite storage、执行迁移，并真实建立 `NativeWsClient` transport 连接资源与 sync 任务句柄；wasm IndexedDB 运行时装配、native 收包/重连后台任务和真实 WebSocket 长连接回归仍待后续 Gate | Session 登录会创建 storage/transport/sync 资源句柄，并能在 Logout 和 UnInit 清理；native SQLite 打开与本地 WebSocket connect/close 已有单元测试覆盖 |
...
- Phase 6 已从资源装配骨架推进到 native 本地资源句柄闭环；session-native 现已在登录阶段真实建立 `NativeWsClient` 连接资源并在退出阶段 close，也已补 `/auth/parse_token` 本地 HTTP 登录校验，但 native WebSocket 收包/重连后台任务、wasm IndexedDB 运行时装配、同步任务和真实 API endpoint 联调仍未接入。
