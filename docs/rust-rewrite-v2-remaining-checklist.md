...
当前仓库的 Rust workspace 已落地核心 crate 列表；绑定层尚未落地，兼容测试 crate 已在 R2-00 继续补齐，并已具备 Go SDK 源码级 public API/listener surface 自动抽取。
...
| 编号 | 阶段 | 状态 | 剩余任务 | 完成标准 |
| --- | --- | --- | --- | --- |
| R2-00 | Phase 0 | 本轮继续推进 | 已补契约冻结报告、Golden Fixture、兼容测试骨架和 Go SDK 源码级 public API/listener surface 自动抽取；仍需补真实 Go SDK 回放、真实服务端 Golden Event、Rust 同场景对比和绑定层回调语义冻结 | 先以本地源码抽取和 fixture 校验通过，后续扩成真实运行回放契约冻结 |
...
- Phase 0 已新增契约冻结骨架，并已补 Go SDK 源码级 public API/listener surface 自动抽取；真实 Go SDK 回放、真实服务端 Golden Event、Rust 同场景对比和绑定层回调语义冻结仍未完成。
...
- Phase 0 的源码级 surface 自动抽取落在兼容测试 crate，当前本机 Go SDK 源码存在时会冻结 134 个 open_im_sdk 导出函数和 14 个 listener interface。
  ...
  <!-- code-ref: v2-phase0-source-extractor -> file:///Volumes/ssd/Users/hj/Documents/code/github/openim/openim-sdk-core-rust/crates/openim-compat-tests/src/lib.rs#L106 -->
...
1. 继续补 Phase 0 真实回放契约：在现有源码级 surface 抽取基础上，补 Go SDK 自动回放、真实服务端 Golden Event、Rust 同场景对比和绑定层回调语义冻结。
...
- 新增 Phase 0 契约冻结文档、兼容测试骨架和 Go SDK 源码级 public API/listener surface 自动抽取。
