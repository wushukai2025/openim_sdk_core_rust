...
当前仓库的 Rust workspace 已落地核心 crate 列表；绑定层尚未落地，兼容测试 crate 已在 R2-00 继续补齐，并已具备 Go SDK 源码级 public API/listener surface 自动抽取、replay transcript 校验入口、绑定回调命名/线程语义冻结、replay-capture transcript 采集工具、Rust 本地 session lifecycle 采集入口和 Go SDK 真实场景回放 harness 源码入口。
...
| 编号 | 阶段 | 状态 | 剩余任务 | 完成标准 |
| --- | --- | --- | --- | --- |
| R2-00 | Phase 0 | 本轮继续推进 | 已补契约冻结报告、Golden Fixture、兼容测试骨架、Go SDK 源码级 public API/listener surface 自动抽取、replay transcript 校验入口、绑定回调命名/线程语义冻结、replay-capture 工具、Rust 本地 session lifecycle 采集入口和 Go SDK 真实场景回放 harness 源码入口；仍需补 Go SDK harness 编译/真实执行、真实服务端 Golden Event 和 Rust 真实服务端同场景采集 | 先以本地源码抽取、fixture 校验、transcript validator、绑定回调契约和采集工具测试通过，后续扩成真实运行回放契约冻结 |
...
- Phase 0 的 replay-capture 工具已加入 workspace，后续真实 Go/Rust 回放器可以通过 stdout JSONL 或 JSONL 文件生成标准 transcript，并用 compare 子命令执行双栈 transcript 对比。
  ...
  <!-- code-ref: v2-phase0-replay-capture-tool -> file:///Volumes/ssd/Users/hj/Documents/code/github/openim/openim-sdk-core-rust/tools/replay-capture/src/main.rs#L95 -->
...
- Phase 0 的 Rust 本地 session lifecycle 采集入口已落在 replay-capture，当前覆盖 init、login、task start/stop、logout 和 uninit。
  ...
  <!-- code-ref: v2-phase0-rust-session-capture -> file:///Volumes/ssd/Users/hj/Documents/code/github/openim/openim-sdk-core-rust/tools/replay-capture/src/main.rs#L159 -->
...
1. 继续补 Phase 0 真实回放契约：在现有源码级 surface 抽取、transcript validator、绑定回调契约、replay-capture 工具和 Go SDK harness 源码入口基础上，补 Go harness 编译/真实执行、真实服务端 Golden Event 和 Rust 真实服务端同场景采集。
...
- 新增 Phase 0 契约冻结文档、兼容测试骨架、Go SDK 源码级 public API/listener surface 自动抽取、replay transcript 校验入口、绑定回调契约、replay-capture 工具、Rust 本地 session lifecycle 采集入口和 Go SDK 真实场景回放 harness 源码入口。
