...
- Phase 6 已从资源装配骨架推进到 native 本地资源句柄闭环；session-native 现已在登录阶段真实建立 `NativeWsClient` 连接资源并在退出阶段 close，但 native WebSocket 收包/重连后台任务、wasm IndexedDB 运行时装配、同步任务和登录 HTTP 校验仍未接入。
