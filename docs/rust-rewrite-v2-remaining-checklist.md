...
| R2-04 | Phase 6 | 本地已推进 | 已接入 SessionRuntimeResources、ResourceOpened/ResourceClosed 事件和 native Session resource adapter；native 登录会打开 SQLite storage、执行迁移，真实建立 NativeWsClient transport 连接资源，并在后台 worker 中轮询收包、断线重连和退出 close；wasm IndexedDB 运行时装配、transport 事件到 session/领域层派发和真实 WebSocket 长连接回归仍待后续 Gate | Session 登录会创建 storage/transport/sync 资源句柄，并能在 Logout 和 UnInit 清理；native SQLite 打开、本地 WebSocket connect/close 与断线重连已有单元测试覆盖 |
...
- Phase 6 已从资源装配骨架推进到 native 本地资源句柄闭环；session-native 现已在登录阶段真实建立 NativeWsClient 连接资源，在后台 worker 中轮询收包并在断线后自动重连，也已补 /auth/parse_token 本地 HTTP 登录校验，但 transport 事件到 session/领域层派发、wasm IndexedDB 运行时装配、同步任务和真实 API endpoint 联调仍未接入。
